...
* Started Vue test adaptation:
  * `starters/vue` runs Vitest + Vue Test Utils interaction tests for `button`, `buttongroup`, `textfield`, `checkbox`, `radio`, `switch`, `tabs`, `tag`, `toast`, `layout`, `labeledvalue`, `illustratedmessage`, and `icon`.
  * Added composition coverage for `accordion`, `actionbar`, `actiongroup`, `breadcrumbs`, `calendar`, `card`, `color`, `contextualhelp`, `datepicker`, `picker`, `dialog`, `overlays`, `menu`, `listbox`, `list`, `tabs`, `tag`, `combobox`, `autocomplete`, `story-utils`, `test-utils`, `theme-dark`, `theme-default`, `theme-express`, `theme-light`, and `style-macro-s1`.
