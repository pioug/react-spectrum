...
* Started Vue test adaptation:
  * `starters/vue` runs Vitest + Vue Test Utils interaction tests for `button`, `buttongroup`, `textfield`, `checkbox`, `radio`, `switch`, `tabs`, `layout`, `labeledvalue`, `illustratedmessage`, and `icon`.
  * Added composition coverage for `accordion`, `actionbar`, `actiongroup`, `breadcrumbs`, `calendar`, `card`, `color`, `contextualhelp`, `datepicker`, `picker`, `dialog`, `overlays`, `menu`, `listbox`, `list`, `tabs`, `combobox`, `autocomplete`, `story-utils`, and `style-macro-s1`.
