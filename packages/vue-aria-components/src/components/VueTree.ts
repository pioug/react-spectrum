...
export const VueTree = defineComponent({
  name: 'VueTree',
  props: {
    items: {
  ...
  setup(props, {emit, attrs}) {
    ...
    let classes = computed(() => ([
      'vs-tree',
      context.value.scale === 'large' ? 'vs-tree--large' : 'vs-tree--medium'
    ]));
    ...
    return function render() {
      return h('nav', {class: [classes.value, attrs.class], 'data-vac': ''}, [renderList(props.items, 'root')]);
    };
  }
});
