...
function normalizeSignatureText(text) {
  ...
  let normalized = text
    ...
    .replace(/Readonly<Ref<boolean>>/g, 'boolean')
    .replace(/Ref<boolean>/g, 'boolean')
    .replace(/\bMaybeElementRef\b/g, 'RefObject<Element | null>')
    .replace(/\bAnyElement\b/g, 'Element | null')
    .replace(/\bAnyRecord\b/g, 'Record<string, unknown>')
