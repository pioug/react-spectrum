...
* Current API coverage: `66.84%` (`2672` source exports, `886` missing exports, `129` fully matched packages).
...
* Recently closed API gaps span 40+ packages across `@react-aria/*`, `@react-spectrum/*`, and `@react-stately/*`; see `migration/VUE_API_PARITY_REPORT.md` for package-level status.
* Remaining API gaps are concentrated in `react-aria-components`, `@react-spectrum/s2`, and 23 medium/small packages.
